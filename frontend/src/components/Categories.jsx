...
function Categories() {
  ...
  const [totalResults, setTotalResults] = useState(0); // Set initial state to 0 for total results
  ...
  var links = [];
  var limit = 2;
  var totalLinks = Math.ceil(totalResults / limit); // Ensure it's rounded up for pagination
  ...
  return (
    <section className="container mt-4">
      ...
      <nav aria-label="Page navigation example" className="mt-4">
        <ul className="pagination"> {/* Use className instead of class */}
          {links}
        </ul>
      </nav>
    </section>
